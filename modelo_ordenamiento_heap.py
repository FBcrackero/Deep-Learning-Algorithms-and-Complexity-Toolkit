...
# --- Entorno RL para Heap Sort (acción: heapify en cualquier índice) ---
class HeapEnv:
    ...
    def heapify(self, n, i):
        ...
        l = 2 * i + 1
        r = 2 * i + 2
        if l < n and self.lista[l] > self.lista[largest]:
            largest = l
        if r < n and self.lista[r] > self.lista[largest]:
            largest = r
        if largest != i:
    ...
    def available_actions(self):
        n = len(self.lista)
        return list(range(n // 2))  # Solo nodos internos pueden ser heapificados

    def step(self, action):
        n = len(self.lista)
        self.heapify(n, action)
        self.steps += 1
        reward = -1
        done = False
        if self.is_sorted():
...
def train_rl(epochs=10000, max_steps=20, n=5, batch_size=32, target_update=500):
    model_path = f"heap_rl_model_{n}_target.pth"
    model = QNet(n)
    target_net = QNet(n)
...
if __name__ == "__main__":
    while True:
        ...
        env = HeapEnv(test_lista)
        state = env.reset()
        print(f"Lista inicial: {state.tolist()}")
        for step in range(30):
            actions = env.available_actions()
            if not actions:
                print("No hay más heapify posibles.")
                break
            state_tensor = torch.tensor(np.array([state]), dtype=torch.float32)
            q_values = model(state_tensor).detach().numpy().flatten()
            action_i = np.argmax(q_values[:len(actions)])
            ...
            print(f"Paso {step+1}: heapify en índice {action}, lista: {state.tolist()}, reward: {reward}")
            if done:
